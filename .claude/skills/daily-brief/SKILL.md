...
**ก่อนอื่นใด — Sync พอร์ตล่าสุด (Full Brief Mode เท่านั้น):**
```bash
git pull origin main
```
- ดึง `portfolio.md` ล่าสุดจาก GitHub ก่อนวิเคราะห์
- ถ้า pull สำเร็จ → แจ้ง "✅ portfolio.md synced"
- ถ้า pull ล้มเหลว (offline / no remote) → ใช้ local ที่มีอยู่ แจ้ง "⚠️ ใช้ portfolio.md local (pull ไม่สำเร็จ)"
- ไม่หยุด pipeline ถ้า pull fail
...
> **ประหยัด token สูงสุด** — พิจารณาทั้ง Holdings เดิม (size up) และ Watchlist (เข้าใหม่) ใน 1 round

**ขั้นตอน:**

1. **อ่าน `portfolio.md`** (local — ไม่ใช้ token search)
   - ดึง Holdings ทุกตัว: ราคาซื้อ, P&L %, thesis status, kill buffer
   - ดึง Watchlist: entry zone ของทุกตัว
   - ดึง Top Pick + thesis สั้น + earnings ที่รออยู่

2. **WebFetch ราคาปัจจุบัน** — พร้อมกันทุกตัว (Holdings + Watchlist Top candidates):
   - Fetch Holdings ทุกตัวจาก Yahoo Finance
   - Fetch Top Pick + Watchlist ที่น่าจะอยู่ใน zone

3. **คัดกรอง 2 กลุ่มพร้อมกัน:**

   **กลุ่ม A — Size Up Holdings เดิม (เพิ่มของเดิม):**
   - Thesis ยังดี (✅ ไม่ใช่ 🚨) AND kill buffer ยังเหลือ >10%
   - ราคาปัจจุบันต่ำกว่าราคาซื้อ ≥5% → น่า average down
   - ยิ่งลงมากและ thesis ไม่พัง = โอกาสดีกว่า

   **กลุ่ม B — เข้าใหม่ Watchlist:**
   - ราคาอยู่ใน entry zone ที่กำหนด
   - Top Pick ก่อนเสมอ → ถ้าไม่อยู่ zone ดูตัวถัดไป

4. **ตัดสิน + ตอบใน chat** (ไม่ save ไฟล์):

   ```
   💰 Top-up แนะนำ — $[จำนวน]

   🔵 SIZE UP (เพิ่มของเดิม):
      SPGI  $411 | ซื้อ $412.79 → ลง 0.4% | thesis ✅ | buffer 17% | โอกาส: ต่ำ (ยังไม่ลงพอ)
      GWRE  $107 | ซื้อ $107.76 → ทรงตัว | thesis ✅ | ❌ ยังไม่น่า average (ไม่ได้ลง)

   🟢 เข้าใหม่ (Watchlist):
      MDLZ $60 | zone $55–$63 ✅ | buffer 4.7% ถึง top | thesis: Oreo/Cadbury moat
      ระวัง: Q2 earnings Jul 28

   📊 แนะนำ: ใส่ $113 ใน MDLZ 100%
      เหตุผล: Holdings ยังไม่ลงพอที่จะ average / MDLZ อยู่ใน zone + thesis intact

   — หรือถ้ามีตัวในกลุ่ม A ที่น่า size up —

   📊 แนะนำแบ่ง:
      GWRE 60% ($68) — ลงมา 8% จากซื้อ + thesis ยัง ARR growth 19%
      MDLZ 40% ($45) — อยู่ใน zone
   ```

   **หลักการตัดสิน:**
   - Holdings ลง ≥8% + thesis ยังดี → size up ก่อน Watchlist
   - Holdings ลง <5% หรือ thesis ⚠️ → ข้ามไป Watchlist
   - 🚨 Kill alert → ห้ามแนะนำ size up เด็ดขาด
   - ถ้าทั้งสองกลุ่มมีตัวดี → แบ่ง % ตาม conviction (buffer เหลือ + thesis strength)
   - ถ้าไม่มีตัวไหนดีเลย → แจ้งตรงๆ "รอก่อน ไม่มีตัวที่น่าเข้าตอนนี้"

> **หลักการ:** fetch ราคาจริงก่อนตัดสิน ไม่ใช้ราคาจาก portfolio.md (อาจ stale) — แต่ไม่ต้องทำ full search news ประหยัด token
