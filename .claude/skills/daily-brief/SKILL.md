...
ใช้ Finviz screener ผ่าน Bash curl (ทดสอบแล้วว่าใช้ได้โดยไม่ต้อง login/paywall — ต้องใส่ User-Agent):
```bash
curl -sL -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36" "https://finviz.com/screener.ashx?v=111&f=cap_midover,fa_salesqoq_o30,geo_usa,sh_avgvol_o500,sh_price_o5&ft=4" -o /tmp/finviz_scan.html
grep -oE 'class="company-ticker" href="stock\?t=[A-Z.\-]+' /tmp/finviz_scan.html | sed 's/.*t=//' | sort -u
```
Filter ที่ใช้: `cap_midover` (mkt cap ≥$2B กันหุ้นเล็ก/ผันผวนเกิน) + `fa_salesqoq_o30` (sales growth QoQ >30% — proxy ของ growth bar ≥30% YoY ใน portfolio.md) + `geo_usa` + `sh_avgvol_o500` (สภาพคล่องพอ) + `sh_price_o5` (กัน penny stock) — ผลลัพธ์แสดง 20 ตัว/หน้า ถ้าอยากได้มากกว่านั้นเติม `&r=21` ต่อ URL เพื่อไปหน้าถัดไป
...
4. โชว์ New Candidates สูงสุด 5 ตัวใน output (เรียงตาม market cap หรือสุ่มถ้าเท่ากัน ไม่ต้อง rank ลึก) พร้อมข้อความแนะนำให้รัน `/brief TICKER` ถ้าสนใจตัวไหน
...
**หมายเหตุ:** Top Pick เลือกจาก **Watchlist เท่านั้น** (ตัวที่ผ่าน `/brief` 4-Layer แล้ว) — ตัวที่เจอจาก Market Scan (ด้านบน) ยังไม่ผ่านการ vet เต็ม จะไม่ถูกเลือกเป็น Top Pick จนกว่าจะรัน `/brief` แล้วเข้า Watchlist ก่อน
...
🆕 Market Scan (ยังไม่ผ่าน /brief): [TICKER1, TICKER2, ...] หรือ "ไม่มีตัวใหม่วันนี้"
