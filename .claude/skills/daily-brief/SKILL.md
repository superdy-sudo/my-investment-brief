...
- **Tier 1 — Buy/Starter อยู่แล้ว (ผ่าน 4-Layer มาแล้ว โอกาสสูงสุด):** เรียงย่อยตาม (a) ใกล้ full-Buy แค่ไหน (Buy-Provisional > Starter ที่ราคาใกล้ entry zone/valuation ใกล้ Cheap > Starter ที่ยังติดเงื่อนไขอื่นค้าง เช่น Second Opinion ไม่ผ่าน) แล้วค่อย (b) วันที่ catalyst ใกล้สุด
- **Tier 2 — Watch แต่ Valuation Inconclusive (สองแหล่งต่างกันแต่ยังไม่ปิดประตู Buy):** เรียงตามวันที่ catalyst ใกล้สุด — โอกาสเป็น Buy ขึ้นกับว่า valuation จะบรรจบไปทางถูกไหม
- **Tier 3 — Watch ที่ growth/valuation ไม่ผ่านเกณฑ์เชิงโครงสร้าง (เช่น growth <30% YoY แบบถาวรของธุรกิจ หรือ premium สูงมากต้องรอ correction ใหญ่):** โอกาสต่ำสุดเสมอไม่ว่า catalyst จะใกล้แค่ไหน เพราะวันที่ไม่ได้แก้ปัญหาเชิงโครงสร้าง — วางท้ายตารางเสมอ

ใส่ tier label กำกับสั้นๆ ในบรรทัดด้วย (🟢 ใกล้ Buy / 🟡 Inconclusive / ⚪ โครงสร้างไม่ผ่าน) ให้ user เห็นเหตุผลของอันดับทันที
...
### 4.5 Quality Gate + Knowledge Atomize (2026-07-20) — เฉพาะวันที่มีการเปลี่ยนแปลงจริง

**ไม่ต้องทำทุกวัน** — วันที่ไม่มีอะไรเปลี่ยน (ราคาขยับปกติ, thesis ทุกตัว No Change) ข้ามขั้นนี้ไปเลย ไปที่ step 5 ตรงๆ

**เรียก `vera`** (`subagent_type: vera`, synchronous) เมื่อเข้าเงื่อนไขข้อใดข้อหนึ่ง: 🔧 TP Reset เกิดขึ้น, Top Pick เปลี่ยนตัว/หลุดจาก Buy-Starter, หรือมี 🚨 Kill Condition trigger — ให้อ่าน output แบบ draft ที่กำลังจะส่งแล้วเช็ค math consistency (kill buffer, P&L %, TP ใหม่คำนวณถูกไหม) ก่อน save จริง — ถ้าเจอ 🔴 issue ต้องแก้ก่อนไป step 5

**เรียก `indy`** (`subagent_type: indy`, background ได้) เมื่อ Holding ตัวไหนตอบคำถาม "Thesis เปลี่ยนไหม?" เป็น ⚠️ (ไม่ใช่ ✅ No change) ในขั้นตอนที่ 2 — ให้เขียน atom entry ใหม่เข้า `knowledge/[TICKER].md` + sync `knowledge/INDEX.md` สรุปว่าอะไรเปลี่ยนไปจาก daily-brief วันนี้

### 5. Save + Push

```bash
git add portfolio.md showcase/index.html showcase/briefs.html knowledge/
...
**เรียก `indy`** (background) ถ้า Thesis Check (ข้อ 3) มีตัวไหน Improved/Weakened — เขียน atom entry เข้า `knowledge/[TICKER].md` สรุปสิ่งที่เปลี่ยนจาก Close Brief วันนี้ ถ้า "No material changes" ทั้งหมด ไม่ต้องเรียก
...
3. คัดกรอง:
   ...
   - เข้าใหม่: Watchlist ที่ราคาอยู่ใน entry zone — **เช็ค Holdings Cap ≤5 ตัวก่อนเสมอ (2026-08-04)** นับจำนวน ticker ใน Holdings table ปัจจุบัน ถ้าเต็ม 5 แล้ว → ห้ามแนะนำ "เข้าใหม่" เว้นแต่ตัวใหม่ conviction สูงกว่าตัวที่แย่สุดใน 5 ตัวเดิมชัดเจน (ถ้าเข้าเงื่อนไขนี้ ให้ระบุด้วยว่าแนะนำสลับตัวไหนออก) — ถ้ายังไม่เต็ม 5 แนะนำเข้าใหม่ได้ตามปกติ เอนเอียงไปทาง Size Up ตัวเดิมมากกว่าเปิดตัวใหม่ถ้า conviction ใกล้เคียงกัน
